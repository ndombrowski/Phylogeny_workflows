---
title: "Phylogenetic pipeline for 51 marker set"
...
- best_hmmer.pl (13 columns, relecant columns: 1, 5 (evalue), 6 (bitscore))
- screen_list_new.pl
...
wdir="/export/lv1/user/ndombrowski/Phylogeny_tutorial"

cpus=10

perl_scripts="/export/data01/tools/scripts/perl"

db_path='/export/lv1/user/spang_team/Databases/52_tested_markers/52tested_inclTIGR.hmm'
marker_list='/export/lv1/user/spang_team/Databases/52_tested_markers/52_markers_list'
...
<p>
  <img width=200, height=200, src="/Users/ninadombrowski/Desktop/WorkingDir/Github_Workflows/51_markers/figures/Picture2.png">
</p>
...
<p>
  <img width=200, height=200, src="/Users/ninadombrowski/Desktop/WorkingDir/Github_Workflows/51_markers/figures/Picture1.png">
</p>
...
# 4. create a file we use for renaming
##################################

Notice:

- the file 'renaming' will be later used to generate an ID that can be used to split the genome ID from the contig ID
- The part were we use the renaming step is a bit tedious and we will adjust the pipeline at a later point.


```bash

#create a file for renaming and splitting files after hmmsearchTable
awk -F'\t' -v OFS='\t' '{print $1, $1}' FileLists/Genomes_of_interest  > temp1
...
for sample in `cat  52_markers_list`; do grep "$sample" Hmmersearch/All_markerss_hmm.txt > Hmmersearch/OGs/${sample}.txt; done
...
for sample in `cat 52_markers_list`; do sort -t$'\t' -k4,4gr -k5,5g Hmmersearch/OGs/${sample}* | sort -t$'\t' --stable -u -k1,1 |  sort -t$'\t' -k4,4gr -k5,5g  > Hmmersearch/deduplicated/${sample}.txt ; done >> duplicated.txt
...
for sample in `cat  52_markers_list`; do awk -F'\t' -v OFS='\t' '{print $2 }' Hmmersearch/deduplicated/$sample* > Hmmersearch/protein_list/$sample |LC_ALL=C  sort ; done
...
for sample in `cat  52_markers_list`; do perl $perl_scripts/screen_list_new.pl Hmmersearch/protein_list/$sample faa/All_Genomes.faa keep > Marker_Genes/raw/${sample}.faa; done
...
parallel -j2 'i={}; nice -n 10 mafft-linsi --reorder --thread 4 Marker_Genes/renamed/$i* > Alignment/mafft_linsi/${i}.aln ' ::: `cat 52_markers_list`
...
parallel -j2 'i={}; nice -n 10 java -jar /opt/biolinux/BMGE-1.12/BMGE.jar -i Alignment/mafft_linsi/$i* -t AA -m BLOSUM30 -h 0.55 -of Alignment/BMGE/h0.55/$i ' :::  `cat 52_markers_list` >> trimming.txt
...
perl $perl_scripts/catfasta2phyml.pl -f -c Alignment/BMGE/h0.55/* > Alignment/concatenated/52markers_ALR_DPANN_ArcRefv5_v1.faa
...
parallel -j10 'i={}; iqtree2 -s Alignment/BMGE/h0.55/${i}.faa -m LG+C10+F+R  -nt 4 -wbtl -bb 1000 -bnni -pre Phylogeny/single/${i}' ::: `cat 52_markers_list`
...
cp Alignment/concatenated/52markers_ALR_DPANN_ArcRefv5_v1.faa Phylogeny/concat
cd Phylogeny/concat

#run iqtree
module load iqtree/2.1.1
iqtree2 -s 52markers_ALR_DPANN_ArcRefv5_v1.faa -m LG+C60+F+R -nt AUTO -bb 1000 -alrt 1000

#rename treefile for easier handling (mapping file available for v6 set)
perl $perl_scripts/Replace_tree_names.pl $taxon_mapping 52markers_ALR_DPANN_ArcRefv5_v1.faa.treefile > 52markers_ALR_DPANN_ArcRefv5_v1.faa.treefile_renamed
